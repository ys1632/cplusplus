...
static float g_width             = 512;       // screen width
static float g_height            = 512;       // screen height
...
struct ShaderState {
  ...
  ShaderState(const char* vsfn, const char* fsfn) {
	...
	h_ratio = safe_glGetUniformLocation(h, "ratio");
	h_g_time = safe_glGetUniformLocation(h, "g_time");
...
static void display(void) {
  ...
  safe_glUniform1f(curSS.h_ratio, ratio);

  //send g_time to fshader
  safe_glUniform1f(curSS.h_g_time, g_time);
